...
include '../../fungsi/koneksi.php';
  if (isset($_GET['laporan'])) {
      ...
      if ($laporan == 'kafe') {
          $query = mysqli_query($con, 'SELECT * FROM kafe');
          ...
          if ($jumlah > 0) {
            ...
            <table class="table table-hover" width="80%" align="center" rules="all" border="1">
              ...
              <tr><th>#</th><th>Kode</th><th>Nama</th><th>Alamat</th><th>Jarak</th></tr>
              <?php
              while ($a = mysqli_fetch_array($query)) {
                  echo '<tr><td><img src="../../file/image/thumb_'.$a['gambar'].'" width="80"></td><td>'.$a['kode'].'</td><td>'.$a['nama'].'</td><td>'.$a['alamat'].'</td><td>'.$a['jarak'].'</td></tr>';
              } ?>
            </table>
            <script>print()</script>
            <?php
      ...
      } elseif ($laporan == 'rangking') {
          $query = mysqli_query($con, 'SELECT rangking.*, kafe.nama, kafe.gambar FROM rangking LEFT JOIN kafe ON rangking.kafe =kafe.kode ORDER BY rangking ASC');
          $jumlah = mysqli_num_rows($query);
          if ($jumlah > 0) {
              ?>
          <table class="table table-hover" width="80%" align="center" rules="all" border="1">
              ...
              <tr><th>#</th><th>Kode</th><th>Nama</th><th>Nilai</th><th>Rangking</th></tr>
              <?php
              while ($a = mysqli_fetch_array($query)) {
                  echo '<tr><td><img src="../../file/image/thumb_'.$a['gambar'].'" width="80"></td><td>'.$a['kafe'].'</td><td>'.$a['nama'].'</td><td>'.$a['nilai'].'</td><td>'.$a['rangking'].'</td></tr>';
              } ?>
            </table>
          <br><br>
          <script>print()</script>
    ...
    if ($jumlah > 0) {
        ...
        while ($b = mysqli_fetch_array($sub)) {
            ...
            echo "<th style='text-align:center' width='".(75/mysqli_num_rows($sub))."%'>$b[kode]</th>";
        }
        echo "</tr>";
        $penjumlah = 0;
        $penjumlah1 = 0;
